...
with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
    for ip, current_route_output in current_ip_route_data.items():
        ...
        df_routes = pd.DataFrame({"Original Routes": original_routes, "New Routes": current_routes})
        df_routes.to_excel(writer, sheet_name=f"{sheet_name}_Comparison", index=False)
